...
class RobotConnection(object):
    ...
    PUSH_PORT = CTRL_PORT + 1
    EVENT_PORT = PUSH_PORT + 1
    IP_PORT = 40926

    ROBOT_VIDEO_PORT = 40921
    ROBOT_AUDIO_PORT = 40922
    ROBOT_CTRL_PORT = 40923
    ROBOT_PUSH_PORT = 40924
    ROBOT_EVENT_PORT = 40925
    ROBOT_IP_PORT = 40926
    ...
    def open(self): 
        ...
        try:
            self.ctrl_socket.connect((self.robot_ip, RobotConnection.ROBOT_CTRL_PORT))
            #self.event_socket.connect((self.robot_ip, RobotConnection.ROBOT_EVENT_PORT))
        except Exception as e:
    ...
    def start_video_recv(self):
        ...
        if self.video_socket not in self.cmd_socket_list:
            ...
            try:
                self.video_socket.connect((self.robot_ip, RobotConnection.ROBOT_VIDEO_PORT))
            except Exception as e:
                print('Connection failed, the reason is %s'%e)
                return False 
    ...
    def start_audio_recv(self):
        ...
        if self.audio_socket not in self.cmd_socket_list:
            ...
            try:
                self.audio_socket.connect((self.robot_ip, RobotConnection.ROBOT_AUDIO_PORT))
            except Exception as e:
                print('Connection failed, the reason is %s'%e)
                return False
...
def test():
    ...
    """
    robot = RobotConnection('192.168.1.123')
    robot.open()
